...
static unsigned char *chsw_images;
static unsigned int *image_in;     /* image to be processed */
static unsigned char *imagesw_in;
static unsigned char *image_out;    /* output image */
static unsigned int*hw_image_out; /* output image computed using the hardware */
...
void HWSW_conv2D(const unsigned int *matrix_in, unsigned int *matrix_out) {
...
#define IP_BASEADDR_BLUE 0x40020000
	...
	volatile unsigned int *in_image_red = (unsigned int *)(IP_BASEADDR_RED + XAXIL_CONV2D_BUS1_ADDR_IMAGE_IN_BASE);
    volatile unsigned int *in_image_green = (unsigned int *)(IP_BASEADDR_GREEN + XAXIL_CONV2D_BUS1_ADDR_IMAGE_IN_BASE);
    volatile unsigned int *in_image_blue = (unsigned int *)(IP_BASEADDR_BLUE + XAXIL_CONV2D_BUS1_ADDR_IMAGE_IN_BASE);
    
	volatile unsigned int *out_image_red = (unsigned int*)(IP_BASEADDR_RED + XAXIL_CONV2D_BUS1_ADDR_IMAGE_OUT_BASE);
    volatile unsigned int*out_image_green = (unsigned int*)(IP_BASEADDR_GREEN + XAXIL_CONV2D_BUS1_ADDR_IMAGE_OUT_BASE);
    volatile unsigned int*out_image_blue = (unsigned int*)(IP_BASEADDR_BLUE + XAXIL_CONV2D_BUS1_ADDR_IMAGE_OUT_BASE);
	...
	volatile signed char *weights_red = (signed char *)(IP_BASEADDR_RED + XAXIL_CONV2D_BUS1_ADDR_WEIGHTS_BASE);
    volatile signed char *weights_green = (signed char *)(IP_BASEADDR_GREEN + XAXIL_CONV2D_BUS1_ADDR_WEIGHTS_BASE);
    volatile signed char *weights_blue = (signed char *)(IP_BASEADDR_BLUE + XAXIL_CONV2D_BUS1_ADDR_WEIGHTS_BASE);
    ...
    *bias_hw_blue = bias;
	...
	for (i=0; i<(IMAGE_WIDTH*IMAGE_HEIGHT)/4; i++) {
        ...
        in_image_green[i] = matrix_in[i + (IMAGE_WIDTH*IMAGE_HEIGHT/4)];
        in_image_blue[i] = matrix_in[i + 2*(IMAGE_WIDTH*IMAGE_HEIGHT/4)];
    ...
    *do_conv_blue = 1;
	...
	while (((*do_conv_blue & 2) == 0) || ((*do_conv_red & 2) == 0) || ((*do_conv_green & 2) == 0));

	for (i=0; i<(OUTPUT_HEIGHT*OUTPUT_WIDTH)/4; i++) {
		matrix_out[i] = out_image_red[i];
        matrix_out[i + (OUTPUT_HEIGHT*OUTPUT_WIDTH/4)] = out_image_green[i];
        matrix_out[i + 2*(OUTPUT_HEIGHT*OUTPUT_WIDTH/4)] = out_image_blue[i];
	}     
...
int main() {
    ...
    /* Select input image */
    image_in =(unsigned int *) ch_images + (IMAGE_TO_CONVOLVE - 1) * IMAGE_SIZE;
    imagesw_in = chsw_images + (IMAGE_TO_CONVOLVE - 1) * IMAGE_SIZE;
    ...
    sw_convolution_2D(imagesw_in,
                      image_out);                                    /* Red channel */
    sw_convolution_2D(imagesw_in + IMAGE_HEIGHT * IMAGE_WIDTH,
                      image_out + OUTPUT_HEIGHT * OUTPUT_WIDTH);     /* Green channel */
    sw_convolution_2D(imagesw_in + 2 * IMAGE_HEIGHT * IMAGE_WIDTH,
