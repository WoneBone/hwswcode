...
`define TV_IN_image_out "../tv/cdatafile/c.axil_conv2D.autotvin_image_out.dat"
...
parameter image_in_DEPTH = 1936;
reg [31 : 0] image_in_OPERATE_DEPTH = 0;
parameter image_in_c_bitwidth = 32;
parameter image_out_DEPTH = 1849;
reg [31 : 0] image_out_OPERATE_DEPTH = 0;
parameter image_out_c_bitwidth = 32;
...
parameter image_out_data_in_addr = 16384;
...
reg image_out_write_data_finish;
...
//write image_out reg
reg [31 : 0] write_image_out_count = 0;
reg [31 : 0] image_out_diff_count = 0;
reg write_image_out_run_flag = 0;
reg write_one_image_out_data_done = 0;
//write weights reg
...
assign TRAN_BUS1_write_data_finish = 1 & image_in_write_data_finish & image_out_write_data_finish & weights_write_data_finish & bias_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_image_out_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (image_out_c_bitwidth, image_out_DEPTH, image_out_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_image_out_run_flag <= 1; 
        end
        else if ((write_one_image_out_data_done == 1 && write_image_out_count == image_out_diff_count - 1) || image_out_diff_count == 0) begin
            write_image_out_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_image_out_count = 0;
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_image_out_count = 0;
        end
        if (write_one_image_out_data_done === 1) begin
            write_image_out_count = write_image_out_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        image_out_write_data_finish <= 0;
    end
    else begin
        if (TRAN_BUS1_start_in === 1) begin
            image_out_write_data_finish <= 0;
        end
        if (write_image_out_run_flag == 1 && write_image_out_count == image_out_diff_count) begin
            image_out_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_image_out
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    integer k;
    reg [31 : 0] image_out_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    ...
    while (1) begin
        wait (AESL_ready_reg === 1);
        image_out_diff_count = 0;

        for (k = 0; k < image_out_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (image_out_c_bitwidth < 32) begin
                    image_out_data_tmp_reg = mem_image_out[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < image_out_c_bitwidth) begin
                            image_out_data_tmp_reg[j] = mem_image_out[k][i*32 + j];
                        end
                        else begin
                            image_out_data_tmp_reg[j] = 0;
                        end
                    end
                end
                if(image_mem_image_out[k * four_byte_num  + i]!==image_out_data_tmp_reg) begin
                image_out_diff_count = image_out_diff_count + 1;
                end
            end
        end
...
initial begin : write_image_out
    integer write_image_out_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_image_out_count = 0; check_image_out_count < image_out_OPERATE_DEPTH; check_image_out_count = check_image_out_count + 1) begin
            wait (ongoing_process_number === process_num && process_busy === 0);
            get_ack = 1;
            if (write_image_out_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write image_out data 
                for (i = 0; i < four_byte_num; i = i + 1) begin
                    if (image_out_c_bitwidth < 32) begin
                        image_out_data_tmp_reg = mem_image_out[check_image_out_count];
                    end
                    else begin
                        for (j = 0; j < 32; j = j + 1) begin
                            if (i*32 + j < image_out_c_bitwidth) begin
                                image_out_data_tmp_reg[j] = mem_image_out[check_image_out_count][i*32 + j];
                            end
                            else begin
                                image_out_data_tmp_reg[j] = 0;
                            end
                    ...
                    if(image_mem_image_out[check_image_out_count * four_byte_num  + i]!==image_out_data_tmp_reg) begin
                        image_mem_image_out[check_image_out_count * four_byte_num + i]=image_out_data_tmp_reg;
                        write (image_out_data_in_addr + check_image_out_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, image_out_data_tmp_reg, write_image_out_resp);
                        write_one_image_out_data_done <= 1;
                        @(posedge clk);
                        write_one_image_out_data_done <= 0;
                    end
                end
            end
            process_busy = 0;
...
// Read data from file 
initial begin : read_image_out_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [32 - 1 : 0] token_tmp; 
  //reg [image_out_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  fp = $fopen(`TV_IN_image_out ,"rb"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_image_out); 
      $finish; 
  end 
  $fread(trans_depth,fp);
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < image_out_DEPTH; i = i + 1) begin 
         $fread(token_tmp,fp);
          if (factor == 4) begin
              if (i%factor == 0) begin
                  tmp_cache_mem [7 : 0] = token_tmp;
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_image_out [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
    ...
    if(fp == 0) begin       // Failed to open file
      ...
      for (i = 0; i < (image_out_DEPTH - image_out_DEPTH % factor); i = i + 1) begin
          ...
          if (factor == 1) begin
              ...
              write_binary_image_out(fp,bin_data,32);
          end
      end 
      if (factor == 4) begin
