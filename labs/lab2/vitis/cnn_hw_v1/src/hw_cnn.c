 ...
 */
 ...
 void hw_convolution_3D(const unsigned int *image_in,__int16_t *weights, __int16_t bias, unsigned int *max_out){
	...
	volatile __int16_t *hw_bias = (__int16_t *)(IP_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_BIAS_DATA );
 ...
 void forward_convolutional_layer() {
    ...
    for(int i = 0; i < 43*43; i++){
        matCrelu[i]=fixed2float((int*) fix_out_image,16); 
    } 
 #endif // USE_GEMM
} 
 ...
 int main(int argc, char **argv) {
     ...
     for (int i = FIRST_IMAGE_TO_CLASSIFY - 1; i < FIRST_IMAGE_TO_CLASSIFY + NUMBER_OF_IMAGES_TO_CLASSIFY - 1; i++) {
         ...
         /* normalize to [-1, 1] */
         normalize_image((unsigned char *) image_in, (float*) fp_image,(int*)fix_out_image);
