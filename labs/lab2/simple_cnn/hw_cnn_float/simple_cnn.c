...
void init_memory() {
    ...
    relu_image = (float *) ((unsigned char *) matCbias + MEM_MAT_C_BIAS);
...
int hw_predict_class() {
#if defined(EMBEDDED) && defined(PRINT_TIME_PER_LAYER)
    double t_start = xilGetMilliseconds();
#endif
    hw_forward_convolutional_layer();
#if defined(EMBEDDED) && defined(PRINT_TIME_PER_LAYER)
    double t_conv = xilGetMilliseconds();
#endif
...
void hw_convolution_3D(const float *image_in, const float *weights, float bias, float *out_max){
	volatile float *in_image = (float *)(IP_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_IMAGE_IN_BASE);
	volatile float *out_max = (float *)(IP_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_MAX_OUT_BASE);
	volatile float *out_image = (float *)(IP_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_IMAGE_OUT_BASE);
	volatile float *hw_weights = (float *)(IP_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_WEIGHTS_BASE);
	volatile float hw_bias = (float *)(IP_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_BIAS);
    volatile float *control = (float *)(IP_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_AP_CTRL);
	*hw_bias = bias;
...
void hw_forward_convolutional_layer() {
    for (int i = 0; i < CONV_OFM_NUMBER; i++) {
        float bias = fp_params[i];
        ...
        hw_convolution_3D((float *) fp_image, fp_weights, bias, (float*)relu_image);
...
int main(int argc, char **argv) {
    ...
    for (int i = FIRST_IMAGE_TO_CLASSIFY - 1; i < FIRST_IMAGE_TO_CLASSIFY + NUMBER_OF_IMAGES_TO_CLASSIFY - 1; i++) {
        ...
        int hw_prediction = hw_prediction();
        ...
        printf(hw_prediction == i % N_CLASSES ? "] OK\n\r" : "] Prediction Error\n\r");
    }
}
