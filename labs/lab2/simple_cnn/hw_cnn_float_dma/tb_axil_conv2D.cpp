...
#ifdef HW_IP
    tmp.data = (ap_int<32>)bias;
    tmp.last=(ap_int<1>)1;
    str_in.write(tmp);

    for(int i=0;i<27;i++){
        tmp.data= (ap_int<32>)kernel[i];
        if (i==(27-1)) tmp.last = (ap_int<1>)1;
		else tmp.last = (ap_int<1>)0;
        str_in.write(tmp);
    }    

    for(int i=0;i<IMAGE_HEIGHT*IMAGE_WIDTH*N_CHANNELS;i++){
        tmp.data= (ap_int<32>)image_fp[i];
        if (i==(IMAGE_HEIGHT*IMAGE_WIDTH*N_CHANNELS-1)) tmp.last = (ap_int<1>)1;
		else tmp.last = (ap_int<1>)0;
        str_in.write(tmp);
    }
    axil_conv2D(str_in, str_out);

    for (int i=0; i<MAX_OUT_SIZE*MAX_OUT_SIZE; i++) {
		tmpa = str_out.read();
		
		max_out[i] = (float)tmpa.data;
	}
